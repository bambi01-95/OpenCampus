...
- �️ **プロジェクト全体表示**: 各プログラムの出席者一覧を詳細表示
- �📤 **データ出力**: Excel/CSV 形式でのデータエクスポート（CS 希望情報含む）
- 📥 **データ入力**: Excel/CSV 形式でのデータインポート（新旧フォーマット対応）
- 🎯 **CS 希望表示**: 参加者の第一～第三希望を表示
...
- **新フォーマット（推奨）**: 姓、名、セイ、メイ、CS 第一希望、CS 第二希望、CS 第三希望を含む
- **旧フォーマット（互換）**: 名前（漢字）、名前（カナ）、プログラム、出席状況（オプション）

#### サンプルフォーマット

**新フォーマット（推奨）:**
| No | 姓 | 名 | セイ | メイ | 高校名 | 高校名カナ | 高校コード | 都道府県 | 設置区分 | 学年 | 性別 | メールアドレス | 同伴者 | CS 第一希望 | CS 第二希望 | CS 第三希望 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 山田 | 太郎 | ヤマダ | タロウ | 京都高校 | キョウトコウコウ | 001 | 京都府 | 私立 | 3 | 男 | yamada@example.com | なし | 目指せ、最速ロボット！　～自動走行プログラミングにトライ～ | 電気の不思議を探ろう！　～ LED を回路の工夫で光らせよう～ | 希望なし |
...
1. 各プログラムカードの「全体表示」ボタンをクリック
2. プログラム名と統計情報が表示される
3. 出席者一覧（番号付き）で参加者を確認
4. CS 希望情報と登録タイプも表示される
...
- **v1.2.0** (2025-01-13): プロジェクト全体表示機能追加
  - 各プログラムカードに「全体表示」ボタンを追加
  - プログラム詳細モーダルで出席者一覧を詳細表示
  - CS 希望情報と登録タイプを含む出席者情報の表示
  - レスポンシブデザイン対応
- **v1.1.0** (2025-01-13): 新フォーマット対応
  - 新しい Excel フォーマット（姓、名、セイ、メイ、CS 希望）に対応
  - CS 第一～第三希望の表示機能追加
  - プロジェクト名を実際のプログラム名に更新
  - 旧フォーマットとの互換性維持
